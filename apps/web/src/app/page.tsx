...
export default function Home() {
  ...
  return (
    <main className={styles.preview}>
      <section className={styles.phone} aria-label="DAY-JA-VIEW 모바일 목업">
        {isLoading ? (
      ...
      {!isLoading && (
        <section className={styles.phone} aria-label="원전수출 테마 상세 목업">
          <div className={styles.detail}>
            <header className={styles.detailHeader}>
              <button type="button" aria-label="뒤로 가기">←</button>
            ...
            <div className={styles.detailScroll}>
              ...
              <section>
                ...
                <ol className={styles.catalysts}>
                  <li><b>1</b><div><strong>체코 원전 수주</strong><p>오늘 뉴스 3건 · 관련 종목 12개</p></div><span>높음</span></li>
                  <li><b>2</b><div><strong>원전 수출 정책</strong><p>오늘 뉴스 2건 · 관련 종목 8개</p></div><span>중간</span></li>
                  <li><b>3</b><div><strong>원전 기자재 공급</strong><p>오늘 뉴스 2건 · 관련 종목 6개</p></div><span>중간</span></li>
                </ol>
              </section>
              ...
              <section>
                ...
                <div className={styles.leaders}>
                  <div><span>두산에너빌리티<small>거래대금 6,240억</small></span><strong>+14.2%</strong></div>
                  <div><span>한전기술<small>거래대금 1,820억</small></span><strong>+7.1%</strong></div>
                  <div><span>우리기술<small>거래대금 940억</small></span><strong>+5.8%</strong></div>
                </div>
              </section>
              ...
              <section>
                <div className={styles.sectionHeading}><h2>오늘과 비슷했던 과거</h2><button type="button">전체 보기</button></div>
                <div className={styles.cases}>
                  <article><span>2024.07.18 · 유사도 87%</span><strong>체코 원전 우선협상대상자 선정</strong><small>T+5 +4.6%</small></article>
                  <article><span>2023.04.25 · 유사도 74%</span><strong>한미 원전 협력 확대 발표</strong><small>T+5 -1.2%</small></article>
                  <article><span>2022.10.31 · 유사도 69%</span><strong>폴란드 원전 개발계획 협력</strong><small>T+5 +3.3%</small></article>
                </div>
              </section>
      ...
      {!isLoading && <RealtimeThemeScreen />}
      {!isLoading && <CaseListScreen />}
...
function WireHeader({ title }: { title: string }) {
  return <header className={styles.wireHeader}><button type="button">←</button><strong>{title}</strong><button type="button">☆</button></header>;
}
...
function CaseListScreen() {
  return (
    <section className={styles.phone} aria-label="과거 사례 전체보기 와이어프레임">
      <div className={styles.wireScreen}>
        <WireHeader title="과거 사례 전체보기" />
        <div className={styles.wireBody}>
          ...
          <div className={styles.caseList}>{caseRows.map(([date, title, members, result]) => <button type="button" key={date}><span><small>{date}</small><strong>{title}</strong><em>{members}</em></span><b>{result}</b><i>›</i></button>)}</div>
        </div>
      </div>
    </section>
  );
...
function CaseDetailScreen() {
  return (
    <section className={styles.phone} aria-label="과거 개별 사례 와이어프레임">
      <div className={styles.wireScreen}>
        <WireHeader title="과거 사례" />
        <div className={styles.wireBody}>
...
function CatalystDetailScreen() {
  return (
    <section className={styles.phone} aria-label="상승 소재 상세 와이어프레임">
      <div className={styles.wireScreen}>
        <WireHeader title="상승 소재" />
        <div className={styles.wireBody}>
...
function LeaderDetailScreen() {
  return (
    <section className={styles.phone} aria-label="주도 종목 상세 와이어프레임">
      <div className={styles.wireScreen}>
        <WireHeader title="당시 주도 종목" />
        <div className={styles.wireBody}>
