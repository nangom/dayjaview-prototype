    ...
    return (
      <div style={css('position:absolute;inset:0;background:' + p.sheetBg + ';display:flex;flex-direction:column;animation:pushIn .28s ease both')}>
        ...
        <div ref={this.themeScrollRef} style={css('position:relative;z-index:1;flex:1;overflow-y:auto;overflow-anchor:none')}>
          ...
          {/* 아래 모서리는 접힐수록 줄이되 없애지는 않는다. 접힌 상태에서
              값이 크면 기기 프레임의 48px 상단 곡선과 서로 밀어내 좌우가
              잘록해지므로, 곡선 사이에 직선이 40px 넘게 남는 선에서 잡았다. */}
          <div style={{
            ...css('position:sticky;top:0;z-index:2;background:#FF5C00'),
            borderRadius: '0 0 ' + (28 - 16 * t).toFixed(1) + 'px ' + (28 - 16 * t).toFixed(1) + 'px',
